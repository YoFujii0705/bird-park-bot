...
class Scheduler {
    ...
    setupEventBroadcastTask() {
        this.tasks.eventBroadcast = cron.schedule('*/90 * * * *', async () => {
    ...
    try {
        ...
        for (const guildId of allGuildIds) {
            try {
                ...
                const channel = this.findBroadcastChannel(guild);
                if (!channel) {
                    console.log(`⚠️ ${guild.name}: レポート送信チャンネルが見つかりません`);
                    continue;
                }
                
                // サーバー別統計生成
                const serverStats = await this.generateServerWeeklyStats(guildId);
                
                // 他のサーバー情報取得
                const otherServersInfo = await this.getOtherServersInfo(guildId);
                
                // サーバー別レポートEmbed作成
                const embed = this.createServerWeeklyReportEmbed(serverStats, otherServersInfo, guild.name);
                
                await channel.send({ embeds: [embed] });
                console.log(`✅ ${guild.name} に週次レポートを送信しました`);
                
                // サーバー間の送信間隔を空ける
                await new Promise(resolve => setTimeout(resolve, 1000));
                
            } catch (error) {
                console.error(`サーバー ${guildId} の週次レポート送信エラー:`, error);
            }
        }
        
        console.log('✅ 全サーバーの週次レポート投稿完了');
...
// サーバー別週次統計生成
async generateServerWeeklyStats(guildId) {
    const stats = await logger.getServerStats(guildId, 7); // logger.jsに新しいメソッドが必要
    let serverZooStats = {
        totalBirds: 0,
        hungryBirds: 0,
        recentEvents: []
    ...
    // このサーバーの鳥類園統計を取得
    try {
        serverZooStats = zooManager.getStatistics(guildId);
    } catch (error) {
        console.error(`サーバー ${guildId} の鳥類園統計取得エラー:`, error);
    }
...
// 他のサーバー情報取得
async getOtherServersInfo(currentGuildId) {
    const allGuildIds = Array.from(this.client.guilds.cache.keys());
    const otherServers = [];
    
    for (const guildId of allGuildIds) {
        if (guildId === currentGuildId) continue; // 自分のサーバーは除外
        
        try {
            const guild = this.client.guilds.cache.get(guildId);
            if (!guild) continue;
            
            const zooStats = zooManager.getStatistics(guildId);
            const recentEvents = zooStats.recentEvents.slice(-2); // 最新2件
            
            otherServers.push({
                guildName: guild.name,
                guildId: guildId,
                totalBirds: zooStats.totalBirds,
                recentEvents: recentEvents
            });
            
        } catch (error) {
            console.error(`他サーバー ${guildId} の情報取得エラー:`, error);
        }
    }
...
// サーバー別週次レポートEmbed作成
createServerWeeklyReportEmbed(stats, otherServersInfo, serverName) {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '🐦 この鳥類園の鳥', value: `${stats.zooStats.totalBirds}羽`, inline: true },
    ...
    // 他の鳥類園の様子を追加
    if (otherServersInfo.length > 0) {
        let otherZooText = '';
        
        for (const otherServer of otherServersInfo) {
            otherZooText += `**とおいどこかの鳥類園** (${otherServer.totalBirds}羽)\n`;
            
            if (otherServer.recentEvents.length > 0) {
                const recentEvent = otherServer.recentEvents[0]; // 最新1件のみ
                otherZooText += `• ${recentEvent.content}\n`;
            } else {
                otherZooText += `• 静かな一日を過ごしています\n`;
            }
            otherZooText += '\n';
        }
        
        if (otherZooText.trim()) {
            embed.addFields({
                name: '🌸 他の鳥類園の様子です',
                value: otherZooText.trim() || '他の鳥類園も平和に過ごしています',
                inline: false
            });
        }
    }

    embed.setFooter({ text: `${serverName} | 来週もよろしくお願いします！` });
